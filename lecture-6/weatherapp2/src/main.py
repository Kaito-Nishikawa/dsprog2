...
def save_weather(area_code, date, weather, temp_min, temp_max):
    conn = get_db()
    cur = conn.cursor()

    cur.execute("""
        INSERT INTO weather (
            area_code, date, weather, temp_min, temp_max, created_at
        ) VALUES (?, ?, ?, ?, ?, ?)
    """, (
        area_code,
        date,
...
def main(page: ft.Page):
    ...
    def load_weather(area_code):
        ...
        dates = ts_weather["timeDefines"][:3]
        weathers = ts_weather["areas"][0]["weathers"][:3]
        temps_min = ts_temp["areas"][0]["tempsMin"][:3]
        temps_max = ts_temp["areas"][0]["tempsMax"][:3]
                    ...
                    content=ft.Column(
                        ...
                        controls=[
                            ...
                            ft.Row(
                                alignment="center",
                                controls=[
                                    ft.Text(f"{temps_min[i]}°C", color="blue"),
                                    ft.Text(" / "),
                                    ft.Text(f"{temps_max[i]}°C", color="red"),
                                ],
                            ),
                        ],
                    ),
